function createApp()
    ...
    for i = 1:5
        ...
        % 1) Definisci la *vera* action del bottone (callback logica)
        if i == 1
            ...
            logicalCb = @(src,evt) simulatePress(src);  % placeholder
        ...
        btn = uibutton(card, 'Text', btnText, ...
            'FontSize', 12, 'FontName', 'Segoe UI', ...
            'Position', [25, 35, 250, 35], ...
            'Tooltip', tip, 'Enable', enableStr, ...
            'BackgroundColor', [0.94 0.94 0.94]);
    
        % 3) Wrappa la callback logica con l’effetto visivo
...
function pushEffect(btn, actionFcn, src, evt)
    ...
    % Effetto press con ripristino garantito
    ...
    % Esegui l’azione reale del bottone
    try
        actionFcn(src, evt);
    catch ME
...
function openPDF(parentfig)
    % Base del progetto = cartella sopra 'Main'
    ...
    didOpen = false;
    for k = 1:numel(cand)
        if isfile(cand{k}), open(cand{k}); didOpen = true; break; end
    end

    if ~didOpen
        uialert(parentfig, 'Relazione non trovata in /Documentation.', 'File mancante');
    end
end
